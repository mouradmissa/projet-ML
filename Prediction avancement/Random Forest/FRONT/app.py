"""
FRONT — Random Forest (Progress). Lancer : python app.py → http://localhost:5002
Modèles : ../MODEL/
"""
...
def _json_safe(obj):
    """Convertit numpy / types exotiques pour jsonify (évite 500 sur /meta)."""
...
_REQUIRED = (
    MODEL_DIR / "rf_progress_model.pkl",
    MODEL_DIR / "rf_progress_features.pkl",
    MODEL_DIR / "rf_progress_meta.pkl",
)
_missing = [str(p) for p in _REQUIRED if not p.is_file()]
if _missing:
    raise FileNotFoundError(
        "Modele Random Forest Progress introuvable dans MODEL/. Fichiers manquants :\n  - "
        + "\n  - ".join(_missing)
        + "\n\nLancez d'abord :\n  python \""
        + str(MODEL_DIR / "rf_progress_pipeline.py")
        + '"'
    )

model = joblib.load(MODEL_DIR / "rf_progress_model.pkl")
feature_cols: list[str] = joblib.load(MODEL_DIR / "rf_progress_features.pkl")
meta: dict = joblib.load(MODEL_DIR / "rf_progress_meta.pkl")
...
def predict():
    ...
    try:
        ...
        traceback.print_exc()
        return jsonify({"error": str(e), "model": "random_forest"}), 400


if __name__ == "__main__":
    _port = 5002
    print("RF Progress — MODEL_DIR =", MODEL_DIR)
    print("RF Progress — CSV =", DATA_PATH)
    try:
        print("RF Progress — http://127.0.0.1:%d" % _port)
        ...
        if winerr == 10048 or "address already in use" in str(e).lower():
            _port = 5022
            print("Port 5002 deja utilise — nouvel essai sur http://127.0.0.1:%d" % _port)
            app.run(debug=False, host="127.0.0.1", port=_port, use_reloader=False)
        else:
            raise
