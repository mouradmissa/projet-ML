...
print("Scaling : NON (Decision Tree n'en a pas besoin)")
...
# =========================================================
# ETAPE 6 : ENTRAINEMENT DECISION TREE (baseline)
# =========================================================
print("=" * 60)
print("  ETAPE 6 : ENTRAINEMENT DECISION TREE (baseline)")
print("=" * 60)
...
print("Baseline (sans contrainte) -> Accuracy = %.4f" % acc_base)
print("  Profondeur : %d | Feuilles : %d" % (dt_base.get_depth(), dt_base.get_n_leaves()))
print()
...
# =========================================================
# ETAPE 8 : ELAGAGE (ccp_alpha)
# =========================================================
print("=" * 60)
print("  ETAPE 8 : ELAGAGE (ccp_alpha)")
print("=" * 60)

dt_for_pruning = DecisionTreeClassifier(**best_params, random_state=42)
...
for alpha in ccp_alphas:
    dt_tmp = DecisionTreeClassifier(**best_params, ccp_alpha=alpha, random_state=42)
...
print("Meilleur ccp_alpha : %.6f (Accuracy = %.4f)" % (best_alpha, best_alpha_acc))

# =========================================================
# ETAPE 9 : EVALUATION MODELE FINAL
# =========================================================
print("=" * 60)
print("  ETAPE 9 : EVALUATION")
print("=" * 60)

final_params = {**best_params, "ccp_alpha": best_alpha, "random_state": 42}
model = DecisionTreeClassifier(**final_params)
model.fit(X_train, y_train)
y_pred = model.predict(X_test)
acc_final = accuracy_score(y_test, y_pred)

print("Accuracy : %.4f" % acc_final)
print("Profondeur : %d | Feuilles : %d" % (model.get_depth(), model.get_n_leaves()))
print()
...
print("- Scaling : NON (Decision Tree)")
print("- Hyperparametres finaux : %s" % final_params)
print("- Accuracy baseline : %.4f" % acc_base)
print("- Accuracy finale (optimise + elague) : %.4f" % acc_final)
...
print("- Remarque : Decision Tree = arbre unique, sensible au surapprentissage,")
print("  elagage (max_depth + ccp_alpha) applique pour regulariser.")
print("=" * 60)

# =========================================================
# SAUVEGARDE
# =========================================================
joblib.dump(model, os.path.join(MODEL_DIR, "dt_risk_model.pkl"))
joblib.dump(le_target, os.path.join(MODEL_DIR, "dt_risk_label_encoder.pkl"))
joblib.dump(list(X.columns), os.path.join(MODEL_DIR, "dt_risk_features.pkl"))
joblib.dump(top_features, os.path.join(MODEL_DIR, "dt_risk_importances.pkl"))

print()
print("Modele sauvegarde       : dt_risk_model.pkl")
print("LabelEncoder sauvegarde : dt_risk_label_encoder.pkl")
print("Features sauvegardees   : dt_risk_features.pkl")
print("Importances sauvegardees: dt_risk_importances.pkl")
